-- 0004 四层角色权限体系（修正版 - 处理已存在的 policy）
...
-- ========== 4. 更新 RLS 策略（先 drop 再 create，避免冲突）==========
...
drop policy if exists "warehouses: manager write" on public.warehouses;
...
drop policy if exists "locations: manager write" on public.locations;
...
drop policy if exists "stock_moves: read own or manager" on public.stock_moves;
...
drop policy if exists "profiles: admin above all" on public.profiles;
